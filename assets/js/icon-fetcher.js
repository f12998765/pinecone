const IconFetcher = {
  ...
  init() {
    ...
    this._cache = {};
    this._generation = 0;
    PineconeDB.get("pinecone-iconMap")
      .then((data) => {
  ...
  onReady(cb) {
    this._ready ? cb() : this._readyListeners.push(cb);
  },
  onResolved(cb) {
  ...
  resolveDomain(domain) {
    ...
    const promise = this._runChain(domain)
      .then((url) => {
      ...
      .catch(() => {
        ...
        if (this._cache)
          this._cache[domain] = "/assets/images/apple-touch-icon.png";
        this._saveCache();
  ...
  resolveDomains(domains, concurrency = 10) {
    ...
    const next = () =>
      index < unique.length
        ? this.resolveDomain(unique[index++]).finally(next)
        : null;
  ...
  async _runChain(domain) {
    const qs = `_=${Date.now()}`;
    if (await this._checkImg(`https://twenty-icons.com/${domain}?${qs}`))
      return `https://twenty-icons.com/${domain}?${qs}`;
    if (
      await this._checkImg(
        `https://favicon.im/${domain}?throw-error-on-404=true&larger=true&${qs}`,
      )
    )
      return `https://favicon.im/${domain}?throw-error-on-404=true&larger=true&${qs}`;
    const v = await this._tryVemetric(domain);
    if (v) return v;
    if (await this._checkImg(`https://${domain}/apple-touch-icon.png?${qs}`))
      return `https://${domain}/apple-touch-icon.png?${qs}`;
    if (await this._checkImg(`https://${domain}/favicon.ico?${qs}`))
  ...
  async _tryVemetric(domain) {
    ...
    try {
      const res = await fetch(
        `https://favicon.vemetric.com/${domain}?response=json&_=${Date.now()}`,
  ...
  async fetchAllIconOptions(domain, { onProgress, onItem, customSources }) {
    const qs = `_=${Date.now()}`;
    let completed = 0;
    const total = 5 + (customSources?.length || 0);
    const trySource = async (url, label) => {
      if (await this._checkImg(url)) {
        if (onItem) onItem({ url, label });
        return true;
      }
      return false;
    };
    const tick = () => {
      completed++;
      if (onProgress) onProgress(`正在获取图标 (${completed}/${total})...`);
    };
    await Promise.allSettled([
      trySource(
        `https://twenty-icons.com/${domain}?${qs}`,
        "twenty-icons",
      ).then(tick),
      trySource(
        `https://favicon.im/${domain}?throw-error-on-404=true&larger=true&${qs}`,
      ...
      trySource(
        `https://${domain}/apple-touch-icon.png?${qs}`,
        "apple-touch-icon",
      ).then(tick),
      trySource(`https://${domain}/favicon.ico?${qs}`, "favicon.ico").then(
        tick,
      ),
      ...(customSources || []).map((src, i) => {
        const sep = src.includes("?") ? "&" : "?";
        return trySource(
          src.replace(/\{domain\}/g, domain) + `${sep}_cb=${Date.now()}_${i}`,
          `源 ${i + 1}`,
        ).then(tick);
      }),
    ]);
